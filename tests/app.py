...
class Directories:
    def __init__(self, dirs, docs):
        self.dirs = dirs
        self.docs = docs
        self.docs.append({"type": "test_type", "number": "123", "name": "test_uset"})

    def get_documents(self, number):
        for doc in self.docs:
            if number == doc['number']:
                return doc

    def get_dir(self, shelf_number):
        if shelf_number not in self.dirs:
            return False
        return self.dirs[shelf_number]
